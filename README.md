...
## 2026-06-09 更新：進階設備分流技術
本方案採用「策略組分流 (偷天換日法)」+「iptables 精確劫持」，實現電視盒 .182 獨享代理，其餘設備自動直連。

### 核心分流原理
1. **策略組分流**：建立名為 `hl13893j` 的策略組，預設指向 `DIRECT`，確保所有規則默認直連。
2. **iptables 精確劫持**：僅針對 192.168.31.182 執行 `REDIRECT`，其他 IP 通過 `ACCEPT` 跳過透明代理。
